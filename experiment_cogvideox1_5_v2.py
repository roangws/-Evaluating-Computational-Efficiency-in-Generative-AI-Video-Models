...
OUTPUT_VIDEO_DIR = "output_videos_cogvideox1_5"
...
def _to_uint8_frame(frame):
    ...
    if arr_min >= 0.0 and arr_max <= 1.0:
        arr = arr * 255.0
    elif arr_min >= -1.0 and arr_max <= 1.0:
...
def _normalize_video_to_uint8(video):
    ...
    if isinstance(video, list):
        ...
        for frame in video:
            ...
            converted_frames.append(_to_uint8_frame(frame))
    ...
    if arr.ndim == 4:
        frames = [_to_uint8_frame(arr[i]) for i in range(arr.shape[0])]
        return np.stack(frames, axis=0)

    return _to_uint8_frame(arr)
...
def load_model(model_name):
    ...
    try:
        ...
        use_offload = _should_use_offload()

        if use_offload and hasattr(pipeline, "enable_sequential_cpu_offload"):
            pipeline.enable_sequential_cpu_offload()
        elif use_offload and hasattr(pipeline, "enable_model_cpu_offload"):
            pipeline.enable_model_cpu_offload()
        else:
            pipeline = pipeline.to("cuda")

        if use_offload and hasattr(pipeline, "enable_attention_slicing"):
            pipeline.enable_attention_slicing()

        if hasattr(pipeline, "vae"):
            if use_offload and hasattr(pipeline.vae, 'enable_tiling'):
...
def calculate_statistics():
    ...
    if len(df_success['model_name'].unique()) == 2:
        ...
        print("  t-statistic: {:.4f}".format(t_stat_time))
        print("  p-value: {:.6f}".format(p_value_time))
